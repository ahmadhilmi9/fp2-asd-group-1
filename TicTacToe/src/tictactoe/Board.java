...
public class Board {
    ...
    public static final Color COLOR_GRID = new Color(90, 45, 45);  // Dark red grid lines for zombie theme
    ...
    public void paint(Graphics g, int boardSize) {
        int cellSize = boardSize / ROWS; // Uniform cell size based on board size

        // Draw grid lines
        g.setColor(COLOR_GRID);
        for (int row = 1; row < ROWS; ++row) {
            int y = row * cellSize;
            g.fillRect(0, y - GRID_WIDTH_HALF, boardSize, GRID_WIDTH);
        }
        for (int col = 1; col < COLS; ++col) {
            int x = col * cellSize;
            g.fillRect(x - GRID_WIDTH_HALF, 0, GRID_WIDTH, boardSize);
        }
        ...
        for (int row = 0; row < ROWS; ++row) {
            for (int col = 0; col < COLS; ++col) {
                int x = col * cellSize;
                int y = row * cellSize;
                cells[row][col].paint(g, x, y, cellSize, cellSize);
            }
        }
    }
}
